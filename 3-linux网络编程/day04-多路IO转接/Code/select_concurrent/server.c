...
int main(int argc, char *argv[]) {
    ...
    Bind(listenfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)); // 2. bind
    ...
    FD_ZERO(&allset); //把文件描述符集合里所有位清0
    FD_SET(listenfd, &allset); /* 构造select监控文件描述符集 */ // 一开始只监听listenfd
    ...
    while (1) {
        ...
        if (FD_ISSET(listenfd, &rset)) { /* 说明有新的客户端链接请求 */
            ...
            connfd = Accept(listenfd, (struct sockaddr *)&clie_addr, &clie_addr_len); /* Accept 不会阻塞 */ // 建立连接
            printf("received from %s at PORT %d\n", inet_ntop(AF_INET, &clie_addr.sin_addr, str, sizeof(str)),
                   ntohs(clie_addr.sin_port));

            for (i = 0; i < FD_SETSIZE; i++) {
                if (client[i] < 0) { /* 找client[]中没有使用的位置，初值-1 */
                    client[i] = connfd; /* 保存accept返回的文件描述符到client[]里 */ // 新建立的连接client[i]
                    break;
                }
            }
            ...
            FD_SET(connfd, &allset); /* 向监控文件描述符集合allset添加新的文件描述符connfd */ // 添加监听新建立的连接
            if (connfd > maxfd) {
